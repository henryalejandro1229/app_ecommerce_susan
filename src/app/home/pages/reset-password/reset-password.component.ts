import { Component, OnInit } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { HomeService } from '../../services/home.service';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import { showNotifyError, showNotifySuccess, showNotifyWarning } from 'src/app/shared/functions/Utilities';
import { ClienteModelo } from '../../models/home.modelo';

@Component({
  selector: 'app-reset-password',
  templateUrl: './reset-password.component.html',
  styleUrls: ['./reset-password.component.scss'],
})
export class ResetPasswordComponent implements OnInit {
  form: FormGroup;
  clear: boolean = false;
  clearConfirm: boolean = false;
  objCliente!: ClienteModelo;
  pwdsCoinciden = false;
  pwdsValue = false;

  constructor(
    private activatedRouter: ActivatedRoute,
    private _hs: HomeService,
    private readonly _router: Router
  ) {
    this.form = new FormGroup({
      password: new FormControl('', [Validators.required]),
      password2: new FormControl('', [Validators.required]),
    });
    this.activatedRouter.queryParams.subscribe((param) => {
      let id: string = param['id'];
      if (!id) {
        showNotifyError('Acceso denegado');
        this._router.navigate(['/home']);
        return;
      }
      this.consultaInfo(id);
    });
  }

  ngOnInit(): void {}

  consultaInfo(id: string): void {
    this._hs.getUsuario(id).subscribe(
      (res: ClienteModelo[]) => {
        if (!res[0]._id) {
          showNotifyError('Acceso denegado');
          this._router.navigate(['/home']);
          return;
        }
        this.objCliente = res[0];
        console.log(this.objCliente);
      },
      (e) => {
        showNotifyError('Error al consultar información');
      }
    );
  }

  restablecer(): void {
    if (this.form.valid && this.pwdsCoinciden) {
      let id: string = this.objCliente._id.$oid;
      this._hs.updatePwd(id, this.form.value.password).subscribe(
        (res) => {
          showNotifySuccess(
            '¡Su contraseña se actualizó correctamente!'
          );
          this._router.navigate(['/login']);
        },
        (e) => {
          showNotifyError(
            'Error al restablecer contraseña'
          );
        }
      );
      return;
    }
    showNotifyWarning(
      'Complete los datos del formulario para continuar'
    );
  }

  validaPwds(): void {
    console.log(this.form.value.password);
    if (this.form.value.password && this.form.value.password2) {
      this.pwdsValue = true;
      this.pwdsCoinciden =
        this.form.value.password === this.form.value.password2;
      return;
    }
    this.pwdsCoinciden = false;
    this.pwdsValue = false;
    console.log(this.pwdsCoinciden);
  }
}
